...
public class NetworkUtils {
    ...
    /**
     * 判断有无网络在连接状态或者正在连接中（查找网络、校验、获取IP等）。
     *
     * @return boolean 不管wifi，还是mobile net，只有当前在连接状态或者正在连接状态才返回true,反之false。
     */
    ...
    public static boolean printNetworkInfo(Context context) {
        ...
        if (connectivity != null) {
            ...
            CLog.e(TAG, "-------------$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$-------------");
            CLog.e(TAG, "getActiveNetworkInfo: " + in);
            NetworkInfo[] info = connectivity.getAllNetworkInfo();
            if (info != null) {
                for (int i = 0; i < info.length; i++) {
                    // if (info[i].getType() == ConnectivityManager.TYPE_WIFI) {
                    CLog.e(TAG, "NetworkInfo[" + i + "]isAvailable : " + info[i].isAvailable());
                    CLog.e(TAG, "NetworkInfo[" + i + "]isConnected : " + info[i].isConnected());
                    CLog.e(TAG, "NetworkInfo[" + i + "]isConnectedOrConnecting : " + info[i].isConnectedOrConnecting());
                    CLog.e(TAG, "NetworkInfo[" + i + "]: " + info[i]);
                    // }
                }
                CLog.e(TAG, "\n");
            } else {
                CLog.e(TAG, "getAllNetworkInfo is null");
            }
        }
        return false;
    }
    ...
    public static int getConnectedTypeINT(Context context) {
        NetworkInfo net = getConnectivityManager(context).getActiveNetworkInfo();
        if (net != null) {
            CLog.e(TAG, "NetworkInfo: " + net.toString());
            return net.getType();
        }
        return -1;
    }
    ...
    public static String getNetWorkType(Context context) {
        ...
        String type = "unknown";
        if (netWorkType == NetWorkType.Net2G) {
            type = "2G";
        } else if (netWorkType == NetWorkType.Net3G) {
            type = "3G";
        } else if (netWorkType == NetWorkType.Net4G) {
            type = "4G";
        } else if (netWorkType == NetWorkType.Wifi) {
            type = "Wifi";
        } else if (netWorkType == NetWorkType.UnKnown) {
            type = "unknown";
        } else if (netWorkType == NetWorkType.NO) {
            type = "no net";
        }
        return type;
    }
