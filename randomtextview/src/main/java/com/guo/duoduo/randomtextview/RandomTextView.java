...
public class RandomTextView extends FrameLayout
    ...
    public void show(int[] angles, float[] lenFactors) {
        this.removeAllViews();
        ...
        if (width > 0 && height > 0 && vecKeywords != null && vecKeywords.size() > 0) {
           ...
           /* LinkedList<Integer> listX = new LinkedList<>();
            ...
            for (int i = 0; i < size; i++) {
                ...
                int xy[] = determineXY(angles[i], lenFactors[i]);
    ...
    private int[] determineXY(int angle, float factor) {
        int[] arr = new int[4];
        ...
        if (angle >= 0 && angle <= 90) { //第一象限
            arr[IDX_X] = (int) (Math.cos(angle * Math.PI / 180) * factor * (this.getWidth() / 2)) + this.getWidth() / 2 - this.getWidth() / 10; //x=cos(angle) * factor * R;
            arr[IDX_Y] = this.getHeight() / 3 - (int) (Math.sin(angle * Math.PI / 180) * factor * (this.getWidth() / 2));
        } else if (angle > 90 && angle <= 180) { //第二象限
            angle = 180 - angle;
            arr[IDX_X] = this.getWidth() / 2 - this.getWidth()/10 - (int) (Math.cos(angle * Math.PI / 180) * factor * (this.getWidth() / 2));
            arr[IDX_Y] = this.getHeight() / 3 - (int) (Math.sin(angle * Math.PI / 180) * factor * (this.getWidth() / 2));
        ...
        /*switch (pos){
            case 1:  //东
                arr[IDX_X] = this.getWidth() / 2 - this.getWidth()/10 + (int)(((absRssi*1.0f)/100)*(this.getWidth()/2.3));
                arr[IDX_Y] = this.getHeight()/3;
                break;

            case 2: //南
                arr[IDX_X] = this.getWidth() / 2 - this.getWidth()/10;
                arr[IDX_Y] = this.getHeight() / 2 - this.getHeight()/9  + (int)(((absRssi*1.0f)/100)*(this.getHeight()/2.5));
                break;

            case 3: //西
                arr[IDX_X] = this.getWidth() / 2 - this.getWidth()/10 - (int)(((absRssi*1.0f)/100)*(this.getWidth()/2.3));
                arr[IDX_Y] = this.getHeight()/2 - this.getHeight()/8;
                break;

            case 4: //北
                arr[IDX_X] = this.getWidth() / 2 - this.getWidth()/10;
                arr[IDX_Y] = this.getHeight() / 2 - this.getHeight()/9 - (int)(((absRssi*1.0f)/100)*(this.getHeight()/2.4));
                break;
        }*/
        return arr;
