...
/**
 * ClassName: HeaderAndFooterRecyclerViewAdapter <br>
 * Description: RecyclerView添加Header/Footer适配器<br>
 * Author: Cyarie <br>
 * Created: 2016/4/14 17:11 <br>
 * Update Time：<br>
    ...
    public void setAdapter(RecyclerView.Adapter<RecyclerView.ViewHolder> adapter) {
        if (null != adapter) {
            ...
            if (adapter instanceof SwipeMenuAdapter)
                this.mSwipeMenuAdapter = (SwipeMenuAdapter) adapter;
        }
    }
    ...
    public void onBindViewHolder(final RecyclerView.ViewHolder holder, final int position) {
        if (null != mAdapter) {
            if (position >= getHeadersCount() && position < getHeadersCount() + mAdapter.getItemCount()) {
                if (mSwipeMenuAdapter != null) {
                    mSwipeMenuAdapter.onBindViewHolder(holder, position - getHeadersCount(), null);
                } else {
