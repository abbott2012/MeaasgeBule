...
public class NotificationDownload {
     ...
     * @param id            通知的唯一标示ID
     */
    public NotificationDownload(Context context, PendingIntent contentIntent,
                                int id) {
    ...
    public void showCustomizeNotification(int icoId, String titleStr, int layoutId) {
        ...
        notification = new Notification(R.drawable.app_logo, titleStr + "下载中...", when);
    ...
    public void changeProgressStatus(int p) {
        if (notification.contentView != null) {
            if (p == DOWNLOAD_FAIL)
                ...
                notification.contentView.setTextViewText(R.id.tvTip, "正在下载("
                        + p + "%) : ");
    ...
    public void changeNotificationText(String content) {
		...
		/*notification.setLatestEventInfo(mContext, titleStr, content,
                contentIntent);*/       //该方法已不能使用

        Notification.Builder builder = new Notification.Builder(mContext).setSmallIcon(R.drawable.app_logo).setContentTitle(titleStr).setContentText(content).setContentIntent(contentIntent);
