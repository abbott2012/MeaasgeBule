...
/**
 * Created by Marktrace003 on 2016/6/14.
 */
public class LineGridView extends GridView {
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        int column = getWidth() / localView1.getWidth();//计算出一共有多少列，假设有2列
        int childCount = getChildCount();//子view的总数
        //System.out.println("子view的总数childCount==" + childCount);
        Paint localPaint;//画笔
        localPaint = new Paint();
        localPaint.setStyle(Paint.Style.STROKE);
        localPaint.setColor(getContext().getResources().getColor(R.color.olive));//设置画笔的颜色R.color.olive
