 ...
 */
public class CheckPhoneAct extends BaseAct implements View.OnClickListener {
    ...
    private void getCheckCode() {
        ...
        if (TextUtils.isEmpty(phone)) {
            Toast.makeText(CheckPhoneAct.this, "请填写手机号码", Toast.LENGTH_SHORT).show();
            return;
        }

        if (!Tools.isMobileNO(phone)) {
            Toast.makeText(CheckPhoneAct.this, "手机号码格式不正确", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        OkGo.post(Path.CHECK_PHONE_EXIST_PATH).tag(this).params(params).execute(new StringDialogCallback(CheckPhoneAct.this, "正在验证手机...") {
            @Override
            public void onSuccess(String result, Call call, Response response) {
                ...
                if (!TextUtils.isEmpty(result)) {
                    ...
                    if (jsonResult != null && jsonResult.isFlag() && jsonResult.getStatusCode() == 200) {
                        ...
                        Map<String, String> params = new HashMap<String, String>();
                        params.put("mobile", phone);
                        OkGo.post(Path.GET_CHECK_CODE_PATH).tag(this).params(params).execute(new StringDialogCallback(CheckPhoneAct.this,"正在获取验证码..."){
                            @Override
                            public void onSuccess(String s, Call call, Response response) {
                                if (!TextUtils.isEmpty(s)) {
                                    ...
                                    if (jsonResult != null && jsonResult.isFlag() && jsonResult.getStatusCode() == 200) {
                                        timeCount.start();
                                        Toast.makeText(CheckPhoneAct.this, "获取验证码成功", Toast.LENGTH_SHORT).show();
                                    } else if (jsonResult != null && jsonResult.getStatusCode() == 400) {
                                        Toast.makeText(CheckPhoneAct.this, "获取验证码失败", Toast.LENGTH_SHORT).show();
                                    } else {
                                        Toast.makeText(CheckPhoneAct.this, "获取验证码失败", Toast.LENGTH_SHORT).show();
                                    }
                                } else {
                        ...
                        isPhoneExist = false;
                        Toast.makeText(CheckPhoneAct.this, "手机号码未注册", Toast.LENGTH_SHORT).show();
                    } else {
                        isPhoneExist = false;
                        Toast.makeText(CheckPhoneAct.this, "手机号码后台校验失败", Toast.LENGTH_SHORT).show();
                    }
                } else {
                    isPhoneExist = false;
                    Toast.makeText(CheckPhoneAct.this, "手机号码后台校验失败", Toast.LENGTH_SHORT).show();
    ...
    private void sure() {
        ...
        if (TextUtils.isEmpty(messageCode)) {
            Toast.makeText(CheckPhoneAct.this, "请填写验证码", Toast.LENGTH_SHORT).show();
            return;
        }


        if (!isPhoneExist) {
            Toast.makeText(CheckPhoneAct.this, "手机号码还未通过验证", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        OkGo.post(Path.FIND_PWD_CHECK_PATH).tag(this).params(params).execute(new StringDialogCallback(CheckPhoneAct.this, "正在验证...") {
            @Override
            public void onSuccess(String result, Call call, Response response) {
                get_checkcode_btn.setEnabled(true);
                sure_btn.setEnabled(true);
                ...
                if (!TextUtils.isEmpty(result)) {
                    ...
                    if (jsonResult != null && jsonResult.isFlag() && jsonResult.getStatusCode() == 200) {
                        Intent i = new Intent(CheckPhoneAct.this, ChangePwdAct.class);
                        ...
                        Toast.makeText(CheckPhoneAct.this, "验证码填写错误", Toast.LENGTH_SHORT).show();
                    } else {
                        Toast.makeText(CheckPhoneAct.this, "验证失败", Toast.LENGTH_SHORT).show();
                    }
                } else {
                    Toast.makeText(CheckPhoneAct.this, "验证失败", Toast.LENGTH_SHORT).show();
                }
    ...
    class TimeCount extends CountDownTimer {
        ...
        public void onTick(long millisUntilFinished) {
            ...
            //get_checkcode_btn.setBackgroundColor(Color.parseColor("#B6B6D8"));
            get_checkcode_btn.setTextColor(Color.parseColor("#c0c0c0"));
            get_checkcode_btn.setClickable(false);
            get_checkcode_btn.setText("(" + millisUntilFinished / 1000 + ") 秒后可重新发送");
        }

        @Override
        public void onFinish() {
            get_checkcode_btn.setTextColor(Color.BLACK);
            get_checkcode_btn.setText("重新获取");
