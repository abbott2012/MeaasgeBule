 ...
 */
public class TraceAct extends AppCompatActivity implements View.OnClickListener, OnDateSetListener, OnGetRoutePlanResultListener {
...
//        initFirst();
    ...
    private void initView() {
        ...
        if (user.getApproleid() == Constant.NORMAL_USER) { //普通用户
            ...
            if (searchObjectFlag == Constant.SEARCH_CAR_TRACE) { //车
                username_tip_tv.setText("车牌号：");
            ...
            if (searchObjectFlag == Constant.SEARCH_CAR_TRACE) { //车辆轨迹查询根据车牌号来查
                username_tip_tv.setText("车牌号：");
                cno_et.setHint("请输入车牌号");
            } else {
                username_tip_tv.setText("身份证：");
            ...
            switch (msg.what) {
                case ROUTE:
                    ...
                    if (routeIndex == planLatLngPolygons.size() - 1) {
                        ...
                        Toast.makeText(TraceAct.this, "播放完毕", Toast.LENGTH_LONG)
                                .show();
                        routeIndex = 0;
                        if (routeMarker != null) {
                            routeMarker.remove();
    ...
    private void loadMapTrace(List<Location> locations) {
        ...
        if (locations != null && locations.size() < 2) {
            Toast.makeText(TraceAct.this, "当前坐标点数少于2，无法加载轨迹", Toast.LENGTH_SHORT).show();
            return;
        }
    ...
    private void loadTrace() {
        ...
        if (user.getApproleid() == Constant.NORMAL_USER) { //普通用户
            ...
            path = Path.CAR_PERSON_TRACE;
        } else { //管理员
            params.put("orgids", user.getOrgids());
            if (searchObjectFlag == Constant.SEARCH_CAR_TRACE) { //查询车辆轨迹
                params.put("cno", cno_et.getText().toString().trim());
                params.put("usertype", "1");
                path = Path.CAR_TRACE_PATH;
            } else if (searchObjectFlag == Constant.SEARCH_PEOPLE_TRACE) { //查询人员轨迹
                params.put("idcard", cno_et.getText().toString().trim());
                params.put("ptype", "1");
                path = Path.PEOPLE_TRACE_PATH;
            }
        ...
        OkGo.post(path).tag(this).params(params).execute(new StringDialogCallback(TraceAct.this, "正在获取轨迹....") {
            @Override
